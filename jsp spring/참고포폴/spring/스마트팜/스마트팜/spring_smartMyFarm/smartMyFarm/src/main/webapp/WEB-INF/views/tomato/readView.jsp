...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#update_btn").on("click", function(){
			...
			formObj.attr("action", "/tomato/updateView");
		...
		$("#delete_btn").on("click", function(){
			...
			formObj.attr("action", "/tomato/delete");
		...
		$("#list_btn").on("click", function(){
			...
			location.href = "/tomato/list?page=${scri.page}"
...
/* 		$("#replyWriteBtn").on("click", function(){
			...
			formObj.attr("action","/tomato/replyWrite");
			formObj.submit();
		}) */
		//댓글 수정 View
/* 		$("#replyUpdateBtn").on("click", function(){
			...
			if(deleteYN == true){
				location.href = "/tomato/replyUpdateView?no=${read.no}"
					+ "&page=${scri.page}"
					+ "&perPageNum=${scri.perPageNum}"
...
/* 		$("#replyDeleteBtn").on("click", function(){
			...
			if(deleteYN == true){
			swal("삭제!", "삭제되었습니다.", "info");
			formObj.attr("action", "/tomato/replyDelete?no=${read.no}"
					+ "&page=${scri.page}"
					+ "&perPageNum=${scri.perPageNum}"
					+ "&searchType=${scri.searchType}"
					+ "&keyword=${scri.keyword}"
  		...
  		<table class="table table-hover" style="text-align: center; border: 2px solid=#dddddd;">
  			<thead>
				...
				<%-- <fmt:parseDate value='${read.redate}' var='redate' pattern='yyyymmdd'/> --%>
					<tr>
						<td Style="width:20%;">주차</td>
					    <td colspan="2">${read.week}</td>
					</tr>
					<tr>
						<td Style="width:20%;">작성일자</td>
					    <td colspan="2">${read.date}<%-- <fmt:formatDate value="${redate}" pattern="yyyy-MM-dd"/> --%></td>
					</tr>
					<tr>
						<td Style="width:20%;">생장길이</td>
					    <td colspan="2">${read.length}</td>
					</tr>
					<tr>
						<td Style="width:20%;">화방높이</td>
					    <td colspan="2">${read.height}</td>
					</tr>
					<tr>
						<td Style="width:20%;">줄기굵기</td>
					    <td colspan="2">${read.width}</td>
					</tr>
					<tr>
						<td Style="width:20%;">잎길이</td>
					    <td colspan="2">${read.leaf_len}</td>
					</tr>
										<tr>
						<td Style="width:20%;">잎수</td>
					    <td colspan="2">${read.leaf}</td>
					</tr>
					<tr>
						<td Style="width:20%;">착과군</td>
					    <td colspan="2">${read.fruit_group}</td>
					</tr>
					<tr>
						<td Style="width:20%;">개화군</td>
					    <td colspan="2">${read.flower}</td>
					</tr>
					<tr>
						<td Style="width:20%;">열매수</td>
					    <td colspan="2">${read.fruit}</td>
					</tr>
				</tbody>
  		</table>
  		<div>
